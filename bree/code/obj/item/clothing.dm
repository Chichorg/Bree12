/obj/item/clothing/under/bree
	icon = 'bree/icons/obj/item/clothing.dmi'
	gender = PLURAL
	body_parts_covered = UPPER_TORSO|LOWER_TORSO|LEGS|ARMS
	item_icons = list(
		slot_w_uniform_str = 'bree/icons/mob/onmob/under.dmi',
		slot_l_hand_str = 'bree/icons/mob/onmob/lefthand.dmi',
		slot_r_hand_str = 'bree/icons/mob/onmob/righthand.dmi'
	)
	has_sensor = SUIT_NO_SENSORS
	sprite_sheets = list()

/obj/item/clothing/under/bree/everyday
	name = "everyday wear"
	desc = ""
	icon_state = "everyday"

/obj/item/clothing/under/bree/toga
	name = "toga"
	desc = ""
	icon_state = "toga"
	body_parts_covered = UPPER_TORSO|LOWER_TORSO|LEGS

/obj/item/clothing/under/bree/pants
	name = "pants"
	desc = ""
	icon_state = "pants"
	body_parts_covered = LOWER_TORSO|LEGS

/obj/item/clothing/under/bree/worn_pants
	name = "pants"
	desc = ""
	icon_state = "worn_pants"
	body_parts_covered = LOWER_TORSO|LEGS



/obj/item/clothing/suit/bree
	icon = 'bree/icons/obj/item/clothing.dmi'
	gender = PLURAL
	body_parts_covered = UPPER_TORSO|LOWER_TORSO|ARMS
	item_icons = list(
		slot_wear_suit_str = 'bree/icons/mob/onmob/suit.dmi',
		slot_l_hand_str = 'bree/icons/mob/onmob/lefthand.dmi',
		slot_r_hand_str = 'bree/icons/mob/onmob/righthand.dmi'
	)
	sprite_sheets = list()

/obj/item/clothing/suit/bree/cape
	name = "cape"
	desc = ""
	icon_state = "cape"
	body_parts_covered = UPPER_TORSO

/obj/item/clothing/suit/bree/cloak
	name = "cloak"
	desc = ""
	icon_state = "cloak"

/obj/item/clothing/suit/bree/shawl
	name = "shawl"
	desc = ""
	icon_state = "shawl"
	body_parts_covered = UPPER_TORSO

/obj/item/clothing/suit/bree/small_cloak
	name = "cloak"
	desc = ""
	icon_state = "small_cloak"
	body_parts_covered = UPPER_TORSO

/obj/item/clothing/suit/bree/robe
	name = "robe"
	desc = ""
	icon_state = "robe"
	body_parts_covered = UPPER_TORSO|LOWER_TORSO|ARMS|LEGS

/obj/item/clothing/suit/bree/robe/alt
	icon_state = "robe_alt"
